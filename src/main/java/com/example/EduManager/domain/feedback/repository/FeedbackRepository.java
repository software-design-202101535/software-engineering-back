...
public interface FeedbackRepository extends JpaRepository<Feedback, Long> {
    ...
    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.student.id = :studentId AND f.category = :category")
    List<Feedback> findAllByStudentIdAndCategory(@Param("studentId") Long studentId, @Param("category") FeedbackCategory category);

    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.student.id = :studentId AND f.studentVisible = true")
    List<Feedback> findAllByStudentIdAndStudentVisibleTrue(@Param("studentId") Long studentId);

    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.student.id = :studentId AND f.studentVisible = true AND f.category = :category")
    ...
    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.student.id = :studentId AND f.parentVisible = true")
    List<Feedback> findAllByStudentIdAndParentVisibleTrue(@Param("studentId") Long studentId);

    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.student.id = :studentId AND f.parentVisible = true AND f.category = :category")
    ...
    @Query("SELECT f FROM Feedback f JOIN FETCH f.teacher WHERE f.id = :feedbackId AND f.student.id = :studentId")
